...
- Tanggal: 21-05-2024\
  \
   <img src="./ttd.png" alt="drawing" width="200"/>
...
```sql
SELECT trx.kode as KODE, trx.tanggal as TANGGAL, p.nama as NAMA_PELANGGAN, b.nama as NAMA_BARANG,
    trx.jumlah_barang as JUMLAH_BARANG, b.harga as HARGA_SATUAN, trx.jumlah_barang*b.harga as TOTAL
FROM transaksi trx
JOIN pelanggan p ON trx.kode_pelanggan = p.kode
JOIN barang b ON trx.kode_barang = b.kode
```

### 4d

```sql
SELECT p.nama as NAMA_PELANGGAN, SUM(trx.jumlah_barang) as JUMLAH, SUM(trx.jumlah_barang*b.harga) as TOTAL
FROM transaksi trx
JOIN pelanggan p ON trx.kode_pelanggan = p.kode
JOIN barang b ON trx.kode_barang = b.kode
GROUP BY trx.kode_pelanggan
```
